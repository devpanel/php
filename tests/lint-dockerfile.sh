...
if [[ "$UPDATE_BASELINE" == true ]]; then
  ...
  if [[ "$count" == "0" ]]; then
    ...
    TODO_ENTRY="- [ ] Remove hadolint baseline comparison: delete ${BASELINE##"$REPO_ROOT"/} and the baseline comparison logic from tests/lint-dockerfile.sh. After that, any new hadolint violation will be an immediate CI failure."
